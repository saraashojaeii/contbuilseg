...
class SegFormerTrainer(BaseTrainer):
    ...
    def save_model(self, epoch, final=False):
        """
        Save SegFormer model checkpoint including optimizer state and training history.
    ...
    def load_checkpoint(self, checkpoint_path):
        """
        Load SegFormer model checkpoint including optimizer state and training history.
    ...
    def train(self, num_epochs=100, save_every=1, start_epoch=1):
        """
        ...
        """
        if start_epoch > 1:
            print(f"Resuming SegFormer training from epoch {start_epoch} to {num_epochs}...")
        else:
